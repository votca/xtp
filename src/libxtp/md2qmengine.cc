...
int Md2QmEngine::DetermineAtomNumOffset(
    const csg::Molecule* mol, const std::vector<int>& atom_ids_map) const {
  std::vector<int> IDs;
  ...
  int offset = IDs[0] - atom_ids_map[0];
...
Topology Md2QmEngine::map(const csg::Topology& top) const {
  ...
  std::map<std::string, std::map<int, std::string> > MolToSegMap;

  // which atomids belong to molname
  std::map<std::string, std::vector<int> > MolToAtomIds;
  ...
  for (const csg::Molecule& mol : top) {
    ...
    int IdOffset = DetermineAtomNumOffset(mol, MolToAtomIds[mol.getType()]);
    for (const csg::Bead* bead : mol.getBeads()) {
      Segment* seg =
          segments[MolToSegMap[mol.getType()][bead->getId() - IdOffset]];

      Atom atom(bead->getResidueId(), bead->getType(), bead->getId(),
                bead->getPos() * tools::conv::nm2bohr, bead->getElement());
      seg->push_back(atom);
    }
  }
