...
Fragment::Fragment(Fragment *stencil)
    : _id(stencil->getId()),
      ...
      _top(NULL),
      _mol(NULL),
      _rotateQM2MD(stencil->getRotQM2MD()),
      _CoQM(stencil->getCoQM()),
      _CoMD(stencil->getCoMD()),
      _trihedron(stencil->getTrihedron()) {

  vector<Atom *>::iterator ait;
  for (ait = stencil->Atoms().begin(); ait < stencil->Atoms().end(); ait++) {
...
void Fragment::AddAtom(Atom *atom) {
  ...
  _weights.push_back(atom->getMass());
}
...
void Fragment::RotTransQM2MD() {
  ...
  for (ait = _atoms.begin(); ait < _atoms.end(); ait++) {
    vec newQMPos =
        _rotateQM2MD * ((*ait)->getQMPos() - this->_CoQM) + this->_CoMD;
...
void Fragment::calcPos(string tag) {
  ...
  for (unsigned int i = 0; i < _atoms.size(); i++) {
    if (tag == "MD") {
      pos += _atoms[i]->getPos() * _atoms[i]->getMass();
    } else if (tag == "QM") {
      if (_atoms[i]->HasQMPart())
        pos += _atoms[i]->getQMPos() * _atoms[i]->getMass();
      else
        assert(_atoms[i]->getMass() < 1e-6);
    }
    totWeight += _atoms[i]->getMass();
  }
...
void Fragment::Rigidify(bool Auto) {
  ...
  if (_symmetry == 3) {
    ...
    if (xMD.getX() * xMD.getX() < 1e-6) {
      yMDtmp = vec(1, 0, 0);
    } else if (xMD.getY() * xMD.getY() < 1e-6) {
      yMDtmp = vec(0, 1, 0);
    } else if (xMD.getZ() * xMD.getZ() < 1e-6) {
      yMDtmp = vec(0, 0, 1);
    }
    if (xQM.getX() * xQM.getX() < 1e-6) {
      yQMtmp = vec(1, 0, 0);
    } else if (xQM.getY() * xQM.getY() < 1e-6) {
      yQMtmp = vec(0, 1, 0);
    } else if (xQM.getZ() * xQM.getZ() < 1e-6) {
      yQMtmp = vec(0, 0, 1);
    }
    ...
    if (abs(yMDtmp) < 0.5) {
      ...
      double tmp_y = 1.;
      double tmp_z =
          1 / xMD.getZ() * (-xMD.getX() * tmp_x - xMD.getY() * tmp_y);
    ...
    if (abs(yQMtmp) < 0.5) {
      ...
      double tmp_y = 1.;
      double tmp_z =
          1 / xQM.getZ() * (-xQM.getX() * tmp_x - xQM.getY() * tmp_y);
    ...
    cout << endl
         ...
         << this->getName();
    cout << ". Assuming point particle for mapping. " << endl;
