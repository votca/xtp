...
bool Orbitals::CheckDegeneracy( double _energy_difference ) {
    ...
    if ( votca::tools::globals::verbose ) cout << endl <<"... ... Checking level degeneracy " << endl;
    ...
    while ( it1 !=_mo_energies.end() ) {
        ...
        while (  it2 !=_mo_energies.end() ) {
            ...
            if ( std::abs(energy1 - energy2)*votca::tools::conv::hrt2ev < _energy_difference ) {
...
void Orbitals::SortEnergies(  std::vector<int>* index ) {
    if ( votca::tools::globals::verbose )  cout << "... ... Sorting energies" << endl ;
