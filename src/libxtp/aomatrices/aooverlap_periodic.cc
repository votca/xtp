...
namespace votca {
    namespace xtp {
        ...
        void AOOverlapPeriodic::FillBlock(ub::matrix_range< ub::matrix<double> >& _matrix, const AOShell* _shell_row, const AOShell* _shell_col, AOBasis* ecp) {
            ...
            if (fabs(_box * _box) < 1.e-9) { //tiny box means it's not set
                cerr << "Periodic box not set!" << flush;
                exit(1);
            }
            ...
            //vec _diff = _pos_row - _pos_col;
            //        for(int k=0; k<3; k++)
            //        {
            //            _diff[k] = fmod(_diff[k], _box[k]);
            //            if(_diff[k] > _box[k]*0.5){
            //                    _diff[k] = _box[k]-_diff[k];
            //            }
            //        }
            ...
            for (AOShell::GaussianIterator itr = _shell_row->firstGaussian(); itr != _shell_row->lastGaussian(); ++itr) {
                ...
                for (AOShell::GaussianIterator itc = _shell_col->firstGaussian(); itc != _shell_col->lastGaussian(); ++itc) {
                    ...
                    //loop over periodic images of the column
                    for (int imX = -_nimages; imX <= _nimages; imX++) {
                        for (int imY = -_nimages; imY <= _nimages; imY++) {
                            for (int imZ = -_nimages; imZ <= _nimages; imZ++) {

                                vec image_shift = vec(imX * _box.getX(), imY * _box.getY(), imZ * _box.getZ());
                                vec _diff = _pos_row - _pos_col - image_shift;
                                double _distsq = (_diff * _diff);
                                ...
                                //periodic
                                const double PmA0 = _fak2 * (_decay_col * -_diff.getX());
                                const double PmA1 = _fak2 * (_decay_col * -_diff.getY());
                                const double PmA2 = _fak2 * (_decay_col * -_diff.getZ());

                                const double PmB0 = _fak2 * (_decay_row * _diff.getX());
                                const double PmB1 = _fak2 * (_decay_row * _diff.getY());
                                const double PmB2 = _fak2 * (_decay_row * _diff.getZ());
                                ...
                                _ol.clear();
                            }//periodic images of columns on Z
                        }//periodic images of columns on Y
                    }//periodic images of columns on X
