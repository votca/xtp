...
void Espfit::Fit2Density(std::vector< QMAtom* >& _atomlist, ub::matrix<double> &_dmat, AOBasis &_basis,BasisSet &bs,string gridsize, Orbitals *orb) { 
    ...
    for ( int i = 0 ; i < _grid.getsize(); i++){
        _ESPatGrid(i)=numway.IntegratePotential(_grid.getGrid()[i]*votca::tools::conv::nm2bohr);
        //++show_progress;
    }
    ...
    for ( unsigned j = 0; j < _atomlist.size(); j++){
      ...
      _pos(0) = votca::tools::conv::ang2nm*_atomlist[j]->x;
      _pos(1) = votca::tools::conv::ang2nm*_atomlist[j]->y;
      _pos(2) = votca::tools::conv::ang2nm*_atomlist[j]->z;
...
ub::vector<double> Espfit::EvalNuclearPotential(std::vector< CTP::QMAtom* >& _atoms, Grid _grid) {
    ...
    for (unsigned i = 0; i < _gridpoints.size(); i++) {
        ...
        for (unsigned j = 0; j < _atoms.size(); j++) {
            ...
            double x_j = votca::tools::conv::ang2nm * _atoms[j]->x;
            double y_j = votca::tools::conv::ang2nm * _atoms[j]->y;
            double z_j = votca::tools::conv::ang2nm * _atoms[j]->z;
            if (_ECP) {
                Znuc = _elements.getNucCrgECP(_atoms[j]->type);
            } else {
                Znuc = _elements.getNucCrg(_atoms[j]->type);
            }
            double dist_j = sqrt((x_j - x_k)*(x_j - x_k) + (y_j - y_k)*(y_j - y_k) + (z_j - z_k)*(z_j - z_k)) * votca::tools::conv::nm2bohr;
...
std::vector<double> Espfit::FitPartialCharges( std::vector< ub::vector<double> >& _fitcenters, Grid& _grid, ub::vector<double>& _potential, double& _netcharge ){
    ...
    for ( unsigned _i =0 ; _i < _Amat.size1()-1; _i++){
        ...
        for ( unsigned _j=_i; _j<_Amat.size2()-1; _j++){
            ...
            for ( unsigned _k=0; _k < _gridpoints.size(); _k++){
                ...
                double dist_i = sqrt( (x_i - x_k)*(x_i - x_k) +  (y_i - y_k)*(y_i - y_k) + (z_i - z_k)*(z_i - z_k)     )*votca::tools::conv::nm2bohr;
                double dist_j = sqrt( (x_j - x_k)*(x_j - x_k) +  (y_j - y_k)*(y_j - y_k) + (z_j - z_k)*(z_j - z_k)     )*votca::tools::conv::nm2bohr;
    ...
    for ( unsigned _i =0 ; _i < _fitcenters.size(); _i++){
        ...
        //LOG(CTP::logDEBUG, *_log) << " Center " << _i << " FitCharge: " << _result[_i] << flush;
        _sumcrg += _result[_i];       
    }
    ...
    for ( unsigned _k=0 ; _k < _gridpoints.size(); _k++ ){
        ...
        for ( unsigned _i=0; _i < _fitcenters.size(); _i++ ){
            ...
            double dist =  sqrt( (x_i - x_k)*(x_i - x_k) +  (y_i - y_k)*(y_i - y_k) + (z_i - z_k)*(z_i - z_k)     )*votca::tools::conv::nm2bohr;
