...
//                #pragma omp parallel for reduction(+:result)
         ...
         */
        void NumericalIntegration::PrepKspaceDensity(double boxLen[3], double ext_alpha, std::vector< CTP::QMAtom* > & _local_atomlist, bool ECP, int nK=0){
...
//                }
        ...
        void NumericalIntegration::PrepKspaceDensity_gromacs_like(double boxLen[3], double ext_alpha, std::vector< CTP::QMAtom* > & _local_atomlist, bool ECP, Grid &eval_grid, int nK)
        {
            ...
            for (unsigned i = 0; i < _grid.size(); i++)
            {
                ...
                {
                    double r[3];
                    r[0]=_grid[i][j].grid_x; //grid is in Bohr
                    r[1]=_grid[i][j].grid_y;
                    r[2]=_grid[i][j].grid_z;
                    for (unsigned m = 0; (m < 3); m++)
                    {
            ...
            #pragma omp parallel for
            for (unsigned i = 0; i < _grid.size(); i++) {
                ub::vector<double> R = eval_grid.getGrid()[i]*tools::conv::nm2bohr; //this is in Bohr, eval_grid is in nm
                for (unsigned m = 0; (m < 3); m++)
                {
        ...
        void NumericalIntegration::FillMadelungGrid(double boxLen[3], int natomsonside)
        {
            //fill _Madelung_grid;
            _Madelung_grid.clear();
        ...
        void NumericalIntegration::IntegratePotential_w_PBC_gromacs_like(Grid &eval_grid, double boxLen[3], ub::vector<double>& _ESPatGrid){
            ...
            if(density_set){
                ...
                for (unsigned p = 0; p < _ESPatGrid.size(); p++){
                    ub::vector<double> rvector = eval_grid.getGrid()[p]*tools::conv::nm2bohr; //Bohr
                    for (unsigned i = 0; i < _grid.size(); i++) {
                        for (unsigned j = 0; j < _grid[i].size(); j++) {
